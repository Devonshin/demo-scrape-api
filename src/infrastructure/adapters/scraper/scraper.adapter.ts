...
import {ScraperPort, ScrapedArticle, ScrapeResult,} from '../../../application/ports/out/scraper/scraper.port';
 ...
 */
@Injectable()
class ScraperAdapter implements ScraperPort {

  private readonly logger = new Logger(ScraperAdapter.name);
  private readonly httpClient: HttpClient;

  constructor (
    @Inject('SourceTagRepositoryPort')
    private readonly sourceTagRepository: SourceTagRepositoryPort,
  ) {
  ...
  private checkCrawlingUrl = (source: SourceDomain, uri: string = '') => {
    return `${source.targetUrl}/${uri.length > 0 ? ('?' + uri) : ''}`
      .replace(ScraperAdapter.MULTIPLE_SLASHES_REGEX, '/')
      .replace(ScraperAdapter.MULTIPLE_QUESTION_MARKS_REGEX, '?');
  }
  ...
  private extractDateField (
    ...
    try {
      if (!tagMap.has(ScraperAdapter.PUBLICATION_DATE)) {
        return null;
      }

      const selector = tagMap.get(ScraperAdapter.PUBLICATION_DATE)!;
      let $dateElement: cheerio.Cheerio<any> | undefined;
      if (selector && selector.startsWith(ScraperAdapter.NEXT_FLAG)) {
        $dateElement = $element.next().find(selector.replace(ScraperAdapter.NEXT_FLAG, ""))
      } else {
